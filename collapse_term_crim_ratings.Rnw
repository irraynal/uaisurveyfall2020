...
termsurveyposneg <- read_csv("Finaluaisurveyresults_term_posneg.csv")
View(termsurveyposneg)

crimsurvey <- read_csv("Finaluaisurveyresults_crim.csv")
View(crimsurvey)

crimsurveyposneg <- read_csv("Finaluaisurveyresults_crim_posneg.csv")
View(crimsurveyposneg)
...
termsurveyposneg$ill_positive_t <- as.numeric(termsurveyposneg$ill_positive)
termsurveyposneg$undoc_positive_t <- as.numeric(termsurveyposneg$undoc_positive)
termsurveyposneg$ill_negative_t <- as.numeric(termsurveyposneg$ill_negative)
termsurveyposneg$undoc_negative_t <- as.numeric(termsurveyposneg$undoc_negative)

crimsurveyposneg$control_positive_c <- as.numeric(crimsurveyposneg$control_positive)
crimsurveyposneg$control_negative_c <- as.numeric(crimsurveyposneg$control_negative)
crimsurveyposneg$crim_positive_c <- as.numeric(crimsurveyposneg$crim_positive)
crimsurveyposneg$crim_negative_c <- as.numeric(crimsurveyposneg$crim_negative)
crimsurveyposneg$nocrim_positive_c <- as.numeric(crimsurveyposneg$nocrim_positive)
crimsurveyposneg$nocrim_negative_c <- as.numeric(crimsurveyposneg$nocrim_negative)
...
<<Rchunk62, echo = FALSE, warning = FALSE, fig.keep = TRUE, results = "hide">>=

#comparison graphic: positive

all_means_t = rbind(positive_means_t, negative_means_t)

ggplot(all_means_t) +
        aes(x = variable, y = mean, color = terminology) +
        geom_point(position = position_dodge(width = 0.5)) +
        geom_linerange(aes(x = variable, ymin = conf_lo, ymax = conf_hi),
                       position = position_dodge(width = 0.5)) + ggtitle("Perceived applicability of traits for illegal/undocumented immigrants") + ylim(c(1,5)) + xlab(NULL)
...
TTpositive_t<- t.test(termsurveyposneg$ill_positive_t, termsurveyposneg$undoc_positive_t, data = termsurveyposneg)
...
TTnegative_t<- t.test(termsurveyposneg$ill_negative_t, termsurveyposneg$undoc_negative_t, data = termsurveyposneg)
CInegative_t<-paste("(",round(TTnegative_t$conf.int[1],2),", ",
          round(TTnegative_t$conf.int[2],2),")",sep="")
...
ggplot(positive_means_c) +
        aes(x = variable, y = mean, color = criminality) +
        geom_point(position = position_dodge(width = 0.5)) +
        geom_linerange(aes(x = variable, ymin = conf_lo, ymax = conf_hi),
...
all_means_c = rbind(positive_means_c, negative_means_c)
...
<<Rchunk17, echo = FALSE, warning = FALSE, fig.keep = TRUE, results = "asis">>=

TTpositive_cc<- t.test(crimsurveyposneg$control_positive_c, crimsurveyposneg$crim_positive_c, data = crimsurveyposneg)
CIpositive_cc<-paste("(",round(TTpositive_cc$conf.int[1],2),", ",
          round(TTpositive_cc$conf.int[2],2),")",sep="")
diffpositive_cc<-round(TTpositive_cc$estimate[1]-TTpositive_cc$estimate[2],2)

TTnegative_cc<- t.test(crimsurveyposneg$control_negative_c, crimsurveyposneg$control_negative_c, data = crimsurveyposneg)
CInegative_cc<-paste("(",round(TTnegative_cc$conf.int[1],2),", ",
          round(TTnegative_cc$conf.int[2],2),")",sep="")
diffnegative_cc<-round(TTnegative_cc$estimate[1]-TTnegative_cc$estimate[2],2)

T3_cc <-matrix(c(diffpositive_cc,CIpositive_cc,round(TTpositive_cc$p.value,2),
                diffnegative_cc,CInegative_cc,round(TTnegative_cc$p.value,2)),nrow=2, byrow = TRUE)
colnames(T3_cc)<-c("Diff in means","95 pct CI","p-value")
row.names(T3_cc)<-c("Positive", "Negative")
# take row and col labels from T1, set digits
xtable(T3_cc,auto=TRUE,caption="Difference in means estimates (Control vs. Criminality)")

@

<<Rchunk18, echo = FALSE, warning = FALSE, fig.keep = TRUE, results = "asis">>=

TTpositive_cnc<- t.test(crimsurveyposneg$control_positive_c, crimsurveyposneg$nocrim_positive_c, data = crimsurveyposneg)
CIpositive_cnc<-paste("(",round(TTpositive_cnc$conf.int[1],2),", ",
          round(TTpositive_cnc$conf.int[2],2),")",sep="")
diffpositive_cnc<-round(TTpositive_cnc$estimate[1]-TTpositive_cnc$estimate[2],2)

TTnegative_cnc<- t.test(crimsurveyposneg$control_negative_c, crimsurveyposneg$nocrim_negative_c, data = crimsurveyposneg)
CInegative_cnc<-paste("(",round(TTnegative_cnc$conf.int[1],2),", ",
          round(TTnegative_cnc$conf.int[2],2),")",sep="")
diffnegative_cnc<-round(TTnegative_cnc$estimate[1]-TTnegative_cnc$estimate[2],2)

T3_cnc <-matrix(c(diffpositive_cnc,CIpositive_cnc,round(TTpositive_cnc$p.value,2),
                diffnegative_cnc,CInegative_cnc,round(TTnegative_cnc$p.value,2)),nrow=2, byrow = TRUE)
colnames(T3_cnc)<-c("Diff in means","95 pct CI","p-value")
row.names(T3_cnc)<-c("Positive", "Negative")
# take row and col labels from T1, set digits
xtable(T3_cnc,auto=TRUE,caption="Difference in means estimates (Control vs. No criminality)")
